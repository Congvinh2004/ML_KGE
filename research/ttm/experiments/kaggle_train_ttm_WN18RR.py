...
checkpoint_path = f'{CHECKPOINT_PATH}/transt_wn18rr_l{TRAIN_RUN}.ckpt'
transt.save_checkpoint(checkpoint_path)
print(f"✅ Checkpoint (với số): {checkpoint_path}")

# Cũng lưu vào file mặc định để dễ load
checkpoint_path_default = f'{CHECKPOINT_PATH}/transt_wn18rr.ckpt'
...
# ============================================
# CELL 11: Lưu kết quả vào file (sau khi test xong)
# ============================================
# Cell này sẽ lưu kết quả vào file sau khi CELL 10 đã test xong
...
result_file = f'{RESULTS_PATH}/transt_wn18rr_l{TRAIN_RUN}_results.txt'
...
import csv
csv_file = f'{RESULTS_PATH}/transt_wn18rr_all_results.csv'
...
with open(csv_file, 'a', newline='', encoding='utf-8') as f:
    ...
    if not file_exists:
        writer.writerow(['Run', 'MRR', 'MR', 'Hits@10', 'Hits@3', 'Hits@1', 'Margin', 'Epochs', 'LR', 'Neg_ent'])
...
# Zip checkpoint
checkpoint_zip = f'/kaggle/working/transt_wn18rr_l{TRAIN_RUN}.zip'
with zipfile.ZipFile(checkpoint_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
    zipf.write(checkpoint_path, f'transt_wn18rr_l{TRAIN_RUN}.ckpt')
    zipf.write(result_file, f'transt_wn18rr_l{TRAIN_RUN}_results.txt')
