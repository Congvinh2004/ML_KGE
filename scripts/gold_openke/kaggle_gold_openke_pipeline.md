...
# --- L3 (chất lượng > L2, tránh timeout 12h) ---
# L2 tham chiếu: TRAIN_TIMES=800, NBATCHES=50 → 800×50 = 40k bước mini-batch/“vòng” tổng mỗi epoch là 50 bước.
# L3: NBATCHES=48 → mỗi epoch ~4% ít bước hơn → cùng wall-time có thể tăng epoch; đặt 834×48 ≈ 40.0k bước (hơn L2 một chút) mà thời gian/epoch ngắn hơn → tổng TransE ~cùng L2, dư biên ~≤11.5h.
# Nếu sát giờ: giảm TRAIN_TIMES xuống 800 (vẫn NBATCHES=48) hoặc GOLD_BATCH_SIZE=96.
# GOLD
EPOCH = 1
GOLD_BATCH_SIZE = 128  # 96 / 64 nếu OOM — batch lớn giúp gold.py nhanh hơn, dành thời gian cho TransE
...
OPENKE_DATA = os.path.join(WORK, "data", "FB15K237")

GOLD_LAYOUT = os.path.join(WORK, "gold_data", "FB15K237_openke")
DATASET_TAG = "FB15K237_openke"
MODEL_NAME = "kaggle_run3"  # TSV: FB15K237_openke_kaggle_run3.tsv (khác L1/L2)
RULE_TOP_K = 100
...
def _find_fb15k237_data_dir(search_roots):
    """Thư mục có entity2id.txt (benchmark OpenKE)."""
    rel_paths = (
        os.path.join("FB15K237"),
        os.path.join("benchmarks", "FB15K237"),
        os.path.join("OpenKE-OpenKE-PyTorch", "benchmarks", "FB15K237"),
        os.path.join("OpenKE-PyTorch", "benchmarks", "FB15K237"),
    )
...
# Repo OpenKE đặt sẵn trong WORK (monorepo: WORK = gốc repo, không có cấp con OpenKE-OpenKE-PyTorch)
if not ON_KAGGLE and os.path.isfile(
    os.path.join(WORK, "scripts", "gold_openke", "openke_to_gold.py")
):
    OPENKE_ROOT = WORK

# --- Nguồn trên Kaggle: một bundle cố định hoặc quét toàn bộ /kaggle/input
search_roots = []
if ON_KAGGLE:
...
src_fb = _find_fb15k237_data_dir(search_roots)
...
# Fallback: benchmark trong working copy OpenKE
bench = os.path.join(OPENKE_ROOT, "benchmarks", "FB15K237")
if not os.path.isfile(os.path.join(OPENKE_DATA, "entity2id.txt")) and os.path.isdir(bench):
    _copytree(bench, OPENKE_DATA)

# Không có dataset FB15K237 trên Kaggle → clone nhẹ nhánh OpenKE-PyTorch (chỉ để lấy benchmarks/FB15K237). Cần Internet.
if ON_KAGGLE and not os.path.isfile(os.path.join(OPENKE_DATA, "entity2id.txt")):
    # Lấy benchmark FB15K237 từ upstream OpenKE nếu user chưa add vào Kaggle input.
    ...
    if need_clone:
        print(
            "Không thấy FB15K237 trong input — clone thunlp/OpenKE (OpenKE-PyTorch) để lấy benchmarks…"
        )
    ...
    # Ưu tiên đúng benchmark FB15K237 (tránh trường hợp scan gặp entity2id.txt của dataset khác như YAGO3-10).
    fb_dir = os.path.join(thunlp_path, "benchmarks", "FB15K237")
    if not os.path.isfile(os.path.join(fb_dir, "entity2id.txt")):
        found = None
        for dp, _, files in os.walk(thunlp_path):
            if os.path.basename(dp) == "FB15K237" and "entity2id.txt" in files:
                found = dp
                break
        fb_dir = found

    if not fb_dir:
        raise RuntimeError("Không tìm thấy benchmark FB15K237 trong clone OpenKE upstream.")

    print("Using upstream benchmark dir:", fb_dir)
    _copytree(fb_dir, OPENKE_DATA)

    # Kiểm tra tối thiểu vài file bắt buộc.
...
print(
    "GOLD-main gold.py:",
    os.path.isfile(os.path.join(GOLD_ROOT, "gold.py")),
)
print(
    "GOLD-main data_loader.py:",
    os.path.isfile(os.path.join(GOLD_ROOT, "data_loader.py")),
)
...
if not os.path.isfile(os.path.join(OPENKE_DATA, "entity2id.txt")):
    missing.append(
        "FB15K237 (entity2id.txt …): thêm Dataset benchmark hoặc repo OpenKE có thư mục benchmarks/FB15K237"
    )
...
if missing:
    ...
    if ON_KAGGLE:
        ...
        kaggle_hint = (
            ...
            "Nếu list rỗng: mở tab notebook → Input (phải) → Add input → Datasets → chọn dataset của bạn rồi Save, sau đó Run lại cell."
            % (exists, inside)
        )
    raise RuntimeError(
        "Thiếu OpenKE/GOLD hoặc benchmark. (Papermill hay ghi In [2] = ô copy/clone repo, không phải ô chạy gold.py.)\n"
        "Xem Markdown đầu (Gắn Input), hoặc bật Internet và đặt GIT_OPENKE / GIT_GOLD ở cell cấu hình.\n\n"
...
```python
# Cell 4 — Cài dependency cho GOLD (không patch runtime)
...
# torch thường có sẵn trên Kaggle GPU (T4). Chỉ cài nếu import lỗi. Không dùng wheel CPU-only — mất GPU.
try:
    import torch
...
# GOLD cần sentence-transformers + tqdm
...
ckpt_dir = os.path.join(OPENKE_ROOT, "checkpoint_gold_clean")
...
_lines = [
    "TransE + link prediction (FB15K237 + GOLD filtered train)",
...
```python
if ON_KAGGLE:
    out_zip = "/kaggle/working/gold_openke_artifact.zip"
